...
#include "../my_socket/client_stream_socket.hpp"
#include "../my_socket/errno_error.hpp"
#include "../my_socket/inet_4_socket_addr.hpp"
#include "../my_socket/inet_6_socket_addr.hpp"
#include "../my_socket/inet_socket_addr.hpp"
#include "../my_socket/socket_error.hpp"
...
irc::core::core(const std::string& server_address, uint16_t port_num, const std::string& nick, const std::string& realname, my_socket::inet_socket_addr::ip_version ip_version/*= inet_socket_addr::ip_version::IPv4*/, const std::string& server_password/*= ""*/)
    : m_server_address{ (
          (ip_version == my_socket::inet_socket_addr::ip_version::IPv4)
          ? (static_cast<my_socket::inet_socket_addr*>(new my_socket::inet4_socket_addr{ server_address, port_num }))
          : (static_cast<my_socket::inet_socket_addr*>(new my_socket::inet6_socket_addr{ server_address, port_num }))
      ) },
      m_nick{ nick },
      m_socket{ my_socket::socket::dummy_type_connect{}, m_server_address },
      m_unread_responses{},
...
irc::core::core(core&& other)
    : m_server_address{ other.m_server_address },
      ...
      m_listener_thread_kill_yourself{ std::move(other.m_listener_thread_kill_yourself.load()) }
{
...
irc::message* irc::core::recv_response()
{
    while (true)
    {
        std::unique_lock<std::mutex> m_unread_responses_mutex_lock{ m_unread_responses_mutex };
        m_unread_responses_cv.wait(m_unread_responses_mutex_lock, [this]{ return m_new_unread_response == true; });
...
void irc::core::send_message_user(const std::string& username, const std::string& realname, const std::pair<user_mode, user_mode> modes/*= {user_mode::MAX, user_mode::MAX}*/)
{
    ...
    auto set_bitmask{ [=, &modes_bitmask](user_mode mode) -> void
    {
        switch (mode)
        {
            ...
            case user_mode::w:
                ...
                break;
            default:
                throw std::runtime_error{ "only user modes 'i' and 'w' may be set through USER command" };
                break;
        }
    } };
    ...
    message.append(std::string{ std::to_string(modes_bitmask) + " * :" + realname });

    send_message(message);
}

void irc::core::send_message_oper(const std::string& name, const std::string& password)
{
    std::string message{ std::string{ "OPER " } + name + " " + password };
    send_message(message);
}

void irc::core::send_message_user_mode(const std::string& nickname, const user_mode mode/*= user_mode::MAX*/, const mode_operation operation/*= mode_operation::MAX*/)
{
    if ((mode != user_mode::MAX) && (operation != mode_operation::MAX))
    {
        ...
        {
            case user_mode::a:
                throw std::runtime_error{ "user mode '[+,-][a]' can only be set through AWAY command" };
                break;
            case user_mode::o:
                [[fallthrough]];
            case user_mode::O:
                switch (operation)
                {
                    case mode_operation::ADD:
                        throw std::runtime_error{ "user mode '[+][o,O]' can only be set through OPER command" };
                        break;
                    default:
                        break;
                }
        ...
        std::string message{ std::string{ "MODE " } + nickname + " " + m_k_mode_operation_lut[static_cast<size_t>(operation)] + m_k_user_mode_lut[static_cast<size_t>(mode)] };
        send_message(message);
    }
    ...
    {
        throw std::runtime_error{ "this should be impossible... someone fucked up their boolean logic" };
    }
}
...
void irc::core::send_message_squit(const std::string& server, const std::string& comment)
{
    std::string message{ std::string{ "SQUIT " } + server + " :" + comment };
    send_message(message);
}

void irc::core::send_message_join(const std::string& channel, const std::string& key/*= ""*/)
{
...
void irc::core::send_message_join(const std::vector<std::pair<std::string, std::string>>& channels_and_keys)
{
...
void irc::core::send_message_part(const std::string& channel, const std::string& part_message/*= ""*/)
{
...
void irc::core::send_message_part(const std::vector<std::string>& channels, const std::string& part_message/*= ""*/)
{
...
void irc::core::send_message_channel_mode(const std::string& channel, const channel_mode mode, const mode_operation operation/*= mode_operation::MAX*/, const std::string& options/*= ""*/)
{
    std::string message{ std::string{ "MODE " } + channel };

    if (operation != mode_operation::MAX)
    {
        message.append(std::string{ " " } + m_k_mode_operation_lut[static_cast<size_t>(operation)]);
    }
...
void irc::core::send_message_topic(const std::string& channel, const std::string& topic/*= ""*/)
{
...
void irc::core::send_message_names(const std::string& channel/*= ""*/, const std::string& target/*= ""*/)
{
...
void irc::core::send_message_names(const std::vector<std::string>& channels/*= {}*/, const std::string& target/*= ""*/)
{
    std::string message{ "NAMES" };
...
void irc::core::send_message_list(const std::string& channel/*= ""*/, const std::string& target/*= ""*/)
{
...
void irc::core::send_message_list(const std::vector<std::string>& channels/*= {}*/, const std::string& target/*= ""*/)
{
    std::string message{ "LIST" };
...
void irc::core::send_message_invite(const std::string& nickname, const std::string& channel)
{
    std::string message{ "INVITE " + nickname + " " + channel };
    send_message(message);
}

void irc::core::send_message_kick(const std::string& channel, const std::string& user, const std::string& comment/*= ""*/)
{
...
void irc::core::send_message_kick(const std::string& channel, const std::vector<std::string>& users, const std::string& comment/*= ""*/)
{
...
void irc::core::send_message_kick(const std::vector<std::string>& channels, const std::vector<std::string>& users, const std::string& comment/*= ""*/)
{
    if (channels.size() != users.size())
    {
        throw std::runtime_error{ "the number of channel parameters and user parameters must be the same" };
    }
...
void irc::core::send_message_privmsg(const std::string& receiver, const std::string& text_to_be_sent)
{
    std::string message{ std::string{ "PRIVMSG " } + receiver + " :" + text_to_be_sent };
    send_message(message);
}

void irc::core::send_message_notice(const std::string& receiver, const std::string& text_to_be_sent)
{
    std::string message{ std::string{ "PRIVMSG " } + receiver + " :" + text_to_be_sent };
    send_message(message);
}
...
void irc::core::send_message_lusers(const std::string& mask/*= ""*/, const std::string& target/*= ""*/)
{
...
void irc::core::send_message_stats(stats_query query/*= stats_query::MAX*/, const std::string& target/*= ""*/)
{
    std::string message{ "STATS" };

    if (query != stats_query::MAX)
    {
        message.append(std::string{ " " } + m_k_stats_query_lut[static_cast<size_t>(query)]);
...
void irc::core::send_message_links(const std::string& remote_server/*= ""*/, const std::string& server_mask/*= ""*/)
{
...
void irc::core::send_message_connect(const std::string& target_server, const std::string& port, const std::string& remote_server/*= ""*/)
{
...
void irc::core::send_message_servlist(const std::string& mask/*= ""*/, const std::string& type/*= ""*/)
{
...
void irc::core::send_message_squery(const std::string& service_name, const std::string& text)
{
    std::string message{ std::string{ "SQUERY " } + service_name + " " + text };
    send_message(message);
}

void irc::core::send_message_who(const std::string& mask/*= ""*/, bool only_opers/*= false*/)
{
...
void irc::core::send_message_whois(const std::string& mask, const std::string& target/*= ""*/)
{
...
void irc::core::send_message_whois(const std::vector<std::string>& masks, const std::string& target/*= ""*/)
{
    std::string message{ "WHOIS " };
...
void irc::core::send_message_whowas(const std::string& nickname, const int count/*= -1*/, const std::string& target/*= ""*/)
{
...
void irc::core::send_message_whowas(const std::vector<std::string>& nicknames, const int count/*= -1*/, const std::string& target/*= ""*/)
{
...
void irc::core::send_message_kill(const std::string& nickname, const std::string& comment)
{
...
bool irc::core::send_message_pong(const std::string& possible_ping_message)
{
    ...
    {
        if (possible_ping_message.find(std::string{ ping + " :" }) != std::string::npos)
        {
...
void irc::core::send_message_summon(const std::string& user, const std::string& target/*= ""*/, const std::string& channel/*= ""*/)
{
...
void irc::core::send_message_userhost(const std::vector<std::string>& nicknames)
{
    static constexpr size_t max_nick_list_size{ 5 };

    if (nicknames.size() > max_nick_list_size)
    {
        throw std::runtime_error{ "USERHOST can only accept a list of up to 5 nicknames" };
    }
...
void irc::core::listen()
{
    ...
    while (m_listener_thread_kill_yourself == false)
    {
        ...
        std::pair<std::deque<std::string>, std::string> new_messages_raw{ split_messages(received_message) };
...
#ifdef DEBUG
        {
            ...
            {
                std::cout << global::color_codes::foreground_yellow << "[DEBUG]: [RECEIVED MESSAGE]: " << new_messages_raw.first[i] << global::color_codes::reset << '\n';
            }
        }
#endif //DEBUG
        ...
        {
            new_messages_raw.first.erase(new_messages_raw.first.begin() + messages_to_erase[i]);
        }
        ...
        for (size_t i{ 0 }; i < new_messages_parsed.size(); ++i)
        {
            switch (message::check_type(new_messages_raw.first[i]))
            {
                case message::type::STANDARD:
                    new_messages_parsed[i] = new standard_message{ new_messages_raw.first[i] };
                    break;
...

        {
            ...
            m_unread_responses.insert(m_unread_responses.end(), new_messages_parsed.begin(), new_messages_parsed.end());
...
void irc::core::send_message(const std::string& message)
{
    ...
    {
        throw std::runtime_error{ "message is too long (" + std::to_string(message.size()) + " chars)" };
    }
...
#ifdef DEBUG
    {
        std::unique_lock<std::mutex> output_mutex_lock{ global::output_mutex::output_mutex };
        std::cerr << global::color_codes::foreground_red << "[DEBUG]: [SENDING MESSAGE]: " << message << global::color_codes::reset << '\n';
    }
#endif //DEBUG
    m_socket.send(std::string{ message + m_k_delimiter });
}

std::pair<std::deque<std::string>, std::string> irc::core::split_messages(const std::string& messages_combined)
{
    std::string message_fragment{ "" };

    if (messages_combined.ends_with(m_k_delimiter) == false)
    {
        message_fragment = messages_combined.substr((messages_combined.rfind(m_k_delimiter) + m_k_delimiter.size()), std::string::npos);
    }
    ...
    while ((pos_next = messages_combined.find(m_k_delimiter, pos_last)) != std::string::npos)
    {
        messages_split.push_back(messages_combined.substr(pos_last, (pos_next - pos_last)));
