...
const std::unordered_map<int, std::string> irc::numeric_message::m_k_code_string_hash_table{
    { 001, "RPL_WELCOME" },
    { 002, "RPL_YOURHOST" },
    { 003, "RPL_CREATED" },
    { 004, "RPL_MYINFO" },
...
irc::numeric_message::numeric_message()
    : message{},
      ...
      m_code_string{},
      m_is_error{},
      m_unknown_code_policy{},
      m_is_code_known{}
{
}

irc::numeric_message::numeric_message(const std::string& raw_message, const unknown_code_policy unknown_code_policy/*= unknown_code_policy::RELAXED*/)
    : message{ raw_message },
      m_code{},
      m_code_string{},
      m_is_error{},
      m_unknown_code_policy{ unknown_code_policy },
      m_is_code_known{}
{
...
int irc::numeric_message::get_code() const
{
    return m_code;
}
...
bool irc::numeric_message::get_is_error() const
{
    return m_is_error;
}

irc::numeric_message::unknown_code_policy irc::numeric_message::get_unknown_code_policy() const
{
    return m_unknown_code_policy;
}

bool irc::numeric_message::get_is_code_known() const
{
    return m_is_code_known;
}

irc::numeric_message& irc::numeric_message::operator=(const numeric_message& other)
{
    if (this == &other)
    {
        return *this;
...
void irc::numeric_message::parse_code(const std::string& raw_message)
{
    ...
    m_code = std::stoi(raw_message.substr((pos_last + 1), ((pos_cur) - (pos_last + 1))));
    ...
    if (m_k_code_string_hash_table.contains(m_code) == false)
    {
        ...
        switch (m_unknown_code_policy)
        {
            ...
            case unknown_code_policy::STRICT:
                throw std::runtime_error{ "message contains unknown numeric code " + std::to_string(m_code) };
                break;
        }
    }
    else
    ...
    else if (((m_code >= client_only_reply_range_start) && (m_code <= client_only_reply_range_end)) || ((m_code >= command_reply_range_start) && (m_code <= command_reply_range_end)))
    {
        m_is_error = false;
    }
    else
